# MonoTask（フロントエンド / スマートフォン対応・ログイン機能実装済み）
...
## 🔗 成果物リポジトリ一覧（2アキテクチャ構成）
本システムは、可読性と保守性を高めるため、フロントエンドとバックエンドのプログラムを完全に分離したプロ仕様の構成で開発しています。

- **フロントエンド（本リポジトリ）**: React (Vite) による画面・レスポンシブ制御
- **バックエンド（APIサーバー）**: [Java / Spring Boot リポジトリはこちら](https://github.com/naomitakei/monotask-backend)

### 🔐 テスト用アカウント
以下のデモ用認証情報をご入力ください。
- **ユーザー名（Username）**: `admin`
- **パスワード（Password）**: `1234`
※未ログイン状態でのタスク登録・取得画面への直接アクセスは、認証制御により完全遮断されます。
...
3. **入力バリデーションの強化（変更）**
   - 空白記述禁止チェック、および最大100文字制限の厳格なバリデーションチェックをフロントエンド側に実装。
...
| カテゴリ | 技術スタック |
| :--- | :--- |
| **フロントエンド** | React (Vite) / JavaScript / レスポンシブCSS |
| **ツール** | Git / GitHub / VS Code |

## 📊 画面・機能の説明
1. **ログイン画面**：アプリ起動時の初期画面。正規アカウント以外を遮断する認証セキュリティをクリア。
<br><img width="280" alt="Image" src="https://github.com/user-attachments/assets/34d300e6-6aa3-45cc-a7ad-c01a49575ed0" /><br> 
